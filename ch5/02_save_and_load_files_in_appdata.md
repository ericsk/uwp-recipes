...
在這則食譜中，您會瞭解到如何運用 UWP 應用程式的 **AppData** 空間來儲存或讀取檔案。
  ...
  > UWP 應用程式的程式為了安全性，不能隨意在未經使用者授權下存取系統裡的檔案，但是系統為每一個應用程式保留了一個空間可以自由地讀寫檔案，這就是 **AppData** 區域，與 [5.1 - 讀取應用程式套件內檔案的內容](01_load_file_from_app_package.md)提到的應用程式套件空間不同，放在 **AppData** 中的內容是可以讀寫的。**應用程式被移除時，這個資料也會被刪除**。
  > 
  > 而 AppData 下分為 _Local_、_Cache_、_Temp_、以及 _Roaming_ 四種目錄：_Local_ 目錄是存放在本機的資料且可透過雲端備份（如果使用者有設定系統雲端備份）；_Cache_ 是只存在本機的資料；_Temp_ 也是存在本機，但是隨時都有可能被系統清除；_Roaming_ 則是會同步在所有安裝了這個應用程式的裝置上（透過雲端同步) 。
  > 
  > 關於 AppData 的詳細說明，可以參考 MSDN 上的文件[應用程式資料儲存區](https://msdn.microsoft.com/zh-tw/library/windows/apps/hh464917.aspx)。
